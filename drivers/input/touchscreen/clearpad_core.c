...
struct synaptics_tool {
	int SYN_TOOL_FINGER;
	int SYN_TOOL_PEN;
	int SYN_TOOL_GLOVE;
} input_type = {0x01, 0x02, 0x03};
...
static int synaptics_clearpad_set_pen(struct synaptics_clearpad *this)
{
	...
	if(this->pen_only_enabled)	{
		input_type.SYN_TOOL_FINGER = input_type.SYN_TOOL_PEN;
	} else if(!this->pen_enabled || !this->pen_only_enabled)	{
		input_type.SYN_TOOL_FINGER = 0x01;
	}
exit:
	return rc;
}
				    ...
				    struct synaptics_pointer *pointer)
{
	...
	case SYN_FUNCAREA_POINTER:
		...
		if (cur->tool == input_type.SYN_TOOL_PEN) {
			cur->tool = MT_TOOL_PEN;
			idev = this->input_pen;
		} else {
			/* shift range if glove event */
			if (cur->tool == input_type.SYN_TOOL_GLOVE) {
				if (cur->z > 0)
		...
		const u8 *buf, int finger)
{
	...
	/* check finger state */
	if (new_point.tool == input_type.SYN_TOOL_FINGER ||
		(this->glove_enabled && new_point.tool == input_type.SYN_TOOL_GLOVE) ||
		(this->pen_enabled && (new_point.tool == input_type.SYN_TOOL_PEN))) {
					      ...
					      char *buf)
{
	...
	if (!strncmp(attr->attr.name, __stringify(fwinfo), PAGE_SIZE))
		...
		snprintf(buf, PAGE_SIZE,
			"%d", this->pen_enabled);
	else if (!strncmp(attr->attr.name, __stringify(pen_only), PAGE_SIZE))
		snprintf(buf, PAGE_SIZE,
			"%d", this->pen_only_enabled);
...
static ssize_t synaptics_clearpad_pen_only_enabled_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct synaptics_clearpad *this = dev_get_drvdata(dev);
	...
	LOCK(this);
	this->pen_enabled = sysfs_streq(buf, "1");
	this->pen_only_enabled = sysfs_streq(buf, "1");
	rc = synaptics_clearpad_set_pen(this);
	if (rc)
		dev_err(&this->pdev->dev, "%s failed\n", __func__);
	UNLOCK(this);
...
static struct device_attribute clearpad_sysfs_attrs[] = {
	...
	__ATTR(pen_only, S_IRUGO | S_IWUSR, synaptics_clearpad_state_show,
				synaptics_clearpad_pen_only_enabled_store),
